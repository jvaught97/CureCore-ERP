...
interface PremiumPromoKittingTabProps {
  items: PromoKittingItem[];
  filteredItems: PromoKittingItem[];
  loading: boolean;
  searchTerm: string;
  ...
  onAdd: () => void;
  onEdit: (item: PromoKittingItem) => void;
  onDelete?: (item: PromoKittingItem) => void;
}
...
export function PremiumPromoKittingTab({
  items,
  filteredItems,
  loading,
  searchTerm,
  ...
  const categoryOptions = [
    { value: 'all', label: 'All Items', helper: 'Complete catalog' },
    { value: 'promo', label: 'Promo Items', helper: 'Marketing materials' },
    { value: 'kitting', label: 'Kitting', helper: 'Kit components' },
    { value: 'insert', label: 'Inserts', helper: 'Thank you cards, flyers' }
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm uppercase tracking-[0.3em] text-[#48A999]">Promo & Kitting Command</p>
            </div>
            <h1 className={`text-3xl md:text-4xl font-semibold ${textColor} mt-2`}>Promo & Kitting</h1>
            <p className={`${textMuted} text-sm md:text-base`}>Track promotional items and kit components</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={onAdd}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid gap-6 md:grid-cols-3">
        {[
          { label: 'Total Items', value: items.length, helper: 'Active inventory' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`${bgCard} rounded-2xl border ${borderColor} p-6 ${textColor} shadow-lg flex flex-col gap-4 md:flex-row md:items-center md:justify-between`}>
          <div>
            ...
            <p className="text-lg font-semibold">Manage your promo inventory</p>
          </div>
        ...
        <div className={`${bgCard} rounded-2xl border ${borderColor} shadow-xl overflow-hidden`}>
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className={`text-sm ${textMuted}`}>Loading items...</div>
            </div>
          ) : filteredItems.length === 0 ? (
            <div className={`text-center py-12 ${textColor}`}>
              <Gift className={`mx-auto h-16 w-16 ${textLight} mb-4 animate-pulse`} />
              <h3 className="text-lg font-semibold mb-2">No items found</h3>
              <p className={`${textMuted} mb-6`}>
                {searchTerm || categoryFilter !== 'all'
                  ? 'Try adjusting your filters'
                  : 'Get started by adding your first promo or kitting item'}
              </p>
              <button
                onClick={onAdd}
                      ...
                      >
                        ...
                        <td className="relative">
                          ...
                          <div className="px-6 py-4 pl-8">
                            <div className="flex items-center gap-3">
                              <Gift className="w-5 h-5 text-[#48A999]" />
                              <div>

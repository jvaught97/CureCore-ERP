-- =====================================================
-- CureCore ERP: Admin Settings Module
...
-- =====================================================
-- Assuming 'users' table exists from auth-context.tsx
...
CREATE TABLE IF NOT EXISTS activity_log (
  ...
  entity TEXT NOT NULL,           -- e.g., "user", "ingredient_category", "formula_phase"
  entity_id UUID,
  action TEXT NOT NULL,           -- "create" | "update" | "delete" | "invite" | "deactivate"
...
-- =====================================================
-- 5. ROLE PERMISSIONS (Fine-grained access control)
-- =====================================================
CREATE TABLE IF NOT EXISTS role_permissions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  user_id UUID NOT NULL,
  module TEXT NOT NULL,           -- 'inventory' | 'formulations' | 'manufacturing' | 'crm' | 'finance' | 'reports' | 'settings'
...
CREATE TABLE IF NOT EXISTS inventory_prefs (
  ...
  uom JSONB,                      -- optional: store base units etc.
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS finance_prefs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL UNIQUE,
  costing_method TEXT DEFAULT 'WEIGHTED_AVG', -- or FIFO/LIFO
...
CREATE TABLE IF NOT EXISTS notification_prefs (
  ...
  tenant_id UUID NOT NULL UNIQUE,
  email_from TEXT,
  smtp JSONB,                     -- {host, port, user, pass, secure}
  slack_webhook_url TEXT,
  webhook_secret TEXT,
  quiet_hours JSONB,              -- {start:'22:00', end:'07:00', timezone:'America/Chicago'}
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS notification_triggers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  key TEXT NOT NULL,              -- 'low_stock' | 'batch_complete' | 'coa_ready' | 'lead_qualified' | 'opp_stage_change'
...
CREATE TABLE IF NOT EXISTS integration_prefs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL UNIQUE,
  analytics JSONB,                -- {posthog_key, ga_id}
  sentry_dsn TEXT,
  shipping JSONB,
  tax_engine JSONB,
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE POLICY "write_own_tenant_activity_admin"
  ...
  WITH CHECK (
    tenant_id = auth.tenant_id() AND auth.is_admin()
  );

...
-- FIXED VERSION: Adds columns BEFORE updating policies that reference them
-- ===================================================================
...
-- Update existing rows to have default value for is_locked
UPDATE public.rd_formulas SET is_locked = FALSE WHERE is_locked IS NULL;
...
-- NOW SAFE because is_locked column exists
-- ============================================================================
...
CREATE POLICY rd_formulas_mod ON public.rd_formulas
  FOR ALL
  USING (created_by = auth.uid() AND (is_locked = FALSE OR is_locked IS NULL))
  WITH CHECK (created_by = auth.uid() AND (is_locked = FALSE OR is_locked IS NULL));
...
-- Recreate modification policy for formula ingredients
-- Note: We can't check parent formula's is_locked here easily, so just check user
